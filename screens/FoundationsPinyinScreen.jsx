...
function SectionHeader({ emoji, title, sub, open, onToggle, alwaysOpen = false, color }) {
  return (
    <TouchableOpacity
      style={[styles.sectionHeader, color && { borderColor: color, borderWidth: 2 }]}
      onPress={alwaysOpen ? undefined : onToggle}
      activeOpacity={alwaysOpen ? 1 : 0.7}
    >
      <View style={styles.sectionHeaderLeft}>
        <Text style={[styles.sectionTitle, color && { color }]}>{emoji} {title}</Text>
        {sub ? <Text style={styles.sectionSub}>{sub}</Text> : null}
      </View>
      {!alwaysOpen && (
        <Text style={[styles.sectionChevron, color && { color }]}>{open ? '▲' : '▼'}</Text>
      )}
    </TouchableOpacity>
  );
}
    ...
    <ScreenBackground levelId="pinyin">
      <SafeAreaView style={styles.safe}>
        ...
        <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
          ...
          <SectionHeader emoji="🔤" title="Initials (声母)" alwaysOpen color="#1467a3" />
          ...
          {/* ══ SECTION 2 · FINALS (always open) ══════════════════════════════ */}
          <SectionHeader emoji="🎶" title="Finals (韵母)" alwaysOpen color="#296614" />
          <View style={[styles.sectionBody, { paddingTop: 0 }]}>
            <Text style={styles.bodyNote}>
          ...
          <SectionHeader emoji="🎵" title="The Four Tones" alwaysOpen color="#B63E2C" />
          <View style={styles.sectionBody}>
            ...
            {TONES.map((tone) => (
              <TouchableOpacity
                key={tone.num}
                style={[styles.toneCard, { borderColor: tone.color + '88', backgroundColor: CARD_WHITE }]}
...
const styles = StyleSheet.create({
  ...
  introCard: {
    ...
    marginBottom: 24, borderWidth: 1, borderColor: 'rgba(155,104,70,0.20)',
  },
  introEmoji: { fontSize: 40, marginBottom: 8 },
  introTitle: { fontSize: 20, fontWeight: '900', color: DEEP_NAVY, marginBottom: 8, textAlign: 'center' },
  introDesc:  { fontSize: 14, color: SLATE_TEAL, lineHeight: 21, textAlign: 'center' },

  // Context banner
  contextBanner: {
    flexDirection: 'row', alignItems: 'center', gap: 12,
    backgroundColor: CARD_WHITE, borderRadius: 16,
    padding: 14, marginBottom: 24, borderWidth: 1.5, borderColor: 'rgba(155,104,70,0.25)',
  },
  contextBannerEmoji: { fontSize: 20 },
  contextBannerTitle: { fontSize: 14, fontWeight: '700', color: WARM_BROWN, marginBottom: 2 },
  contextBannerSub:   { fontSize: 12, color: SLATE_TEAL },
  contextBannerArrow: { fontSize: 16, color: WARM_BROWN, fontWeight: '700' },
  ...
  sectionTitle:   { fontSize: 17, fontWeight: '800', color: DEEP_NAVY },
  sectionSub:     { fontSize: 11, color: SLATE_TEAL },
  sectionChevron: { fontSize: 12, color: SLATE_TEAL, marginLeft: 8 },

  // Section body (expanded content)
  sectionBody: {
    backgroundColor: CARD_WHITE, borderRadius: 14, marginTop: 2, marginBottom: 4,
    padding: 16, borderWidth: 1, borderColor: 'rgba(155,104,70,0.12)',
  },
  bodyNote:    { fontSize: 13, color: SLATE_TEAL, lineHeight: 19, marginBottom: 16 },
  ...
  toneBadgeNum:  { fontSize: 18, fontWeight: '900', color: CARD_WHITE },
  toneCardBody:  { flex: 1 },
  toneCardTop:   { flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 3 },
  toneMark:      { fontSize: 24, fontWeight: '900' },
  toneName:      { fontSize: 15, fontWeight: '700', color: DEEP_NAVY },
  toneDesc:      { fontSize: 12, color: SLATE_TEAL, marginBottom: 6 },
  toneExampleRow:{ flexDirection: 'row', alignItems: 'center', gap: 8 },
  toneChar:      { fontSize: 20, fontWeight: '900', color: DEEP_NAVY },
  tonePinyin:    { fontSize: 13, color: SLATE_TEAL },
  toneMeaning:   { fontSize: 12, color: SLATE_TEAL, fontStyle: 'italic' },
  audioIcon:     { fontSize: 22 },

  // Tone mark grid
  gridLabel: {
    fontSize: 10, fontWeight: '800', color: SLATE_TEAL,
    letterSpacing: 1.2, marginTop: 20, marginBottom: 10,
  },
  toneGrid:        { backgroundColor: CARD_WHITE, borderRadius: 16, overflow: 'hidden', marginBottom: 16, borderWidth: 1, borderColor: 'rgba(155,104,70,0.15)' },
  toneGridRow:     { flexDirection: 'row', borderBottomWidth: 1, borderBottomColor: 'rgba(155,104,70,0.10)' },
  toneGridCell:    { flex: 1, paddingVertical: 10, textAlign: 'center', fontSize: 14, fontWeight: '700', color: SLATE_TEAL },
  ...
  toneGridVowelLabel:{ fontSize: 15, fontWeight: '900', color: DEEP_NAVY },
  toneGridMark:    { fontSize: 20, fontWeight: '900' },

  // Tip box
  tipBox: {
    backgroundColor: CARD_WHITE, borderRadius: 16,
    padding: 16, borderWidth: 1, borderColor: 'rgba(155,104,70,0.20)', gap: 6,
  },
  tipTitle: { fontSize: 13, fontWeight: '800', color: WARM_BROWN, marginBottom: 4 },
  tipText:  { fontSize: 12, color: SLATE_TEAL, lineHeight: 19 },
  ...
  chipInitialText: { fontSize: 15, fontWeight: '700', color: WARM_BROWN },
  chipFinal:       { backgroundColor: CARD_WHITE, borderRadius: 10, paddingHorizontal: 14, paddingVertical: 8, borderWidth: 1.5, borderColor: 'rgba(55,73,80,0.30)' },
  chipFinalText:   { fontSize: 15, fontWeight: '700', color: SLATE_TEAL },
  ...
  systemBtn: {
    ...
    borderWidth: 2, borderColor: 'rgba(155,104,70,0.35)',
  },
  systemBtnEmoji: { fontSize: 32 },
  systemBtnTitle: { fontSize: 15, fontWeight: '800', color: WARM_BROWN, marginBottom: 2 },
